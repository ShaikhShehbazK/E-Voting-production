...
const UpdateCandidate = ({ mode }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (mode === 'create') {
      try {
        ...
        if (Image) {
          data.append('image', Image); // ✅ Only append if exists
        }

        const response = await axios.post(`${Api}/candidate`, data, header);
      ...
      try {
        ...
        const response = await axios.put(`${Api}/candidate/${candidateId}`, data, header);
        console.log(response.data);
  ...
  return (
    <div className="w-full max-w-md mx-auto mt-10 px-6 py-8 bg-gradient-to-br from-white via-blue-50 to-blue-100 shadow-xl rounded-2xl border border-blue-200">
      {mode === 'create' ? (
        <h1 className="text-2xl font-bold mb-6 text-center text-blue-800">Adding New Candidate</h1>
      ) : (
      ...
      <form className="space-y-5" onSubmit={handleSubmit} encType="multipart/form-data">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="party">
            Party Name
          </label>
          <input
            id="party"
